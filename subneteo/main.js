...
let mascaraA = new RegExp("^255\\.0\\.0\\.0$");
let mascaraB = new RegExp("^255\\.255\\.0\\.0$");
let mascaraC = new RegExp("^255\\.255\\.255\\.0$");
console.log(select);
...
subredes.addEventListener("change", (e) => {
  ...
  if (clase <= 127 ) {
    ...
    host.value = 65536/e.target.value
  }else if (clase <= 223 ) {
    host.value = 256/e.target.value
  }
});
host.addEventListener("change", (e) => {
  ...
  if (clase <= 127 ) {
    ...
    subredes.value = 65536/e.target.value
  }else if (clase <= 223 ) {
    subredes.value = 256/e.target.value
  }
});
...
btn.addEventListener("click", (e) => {
  ...
  if (ipv4.test(ip.value) && ipv4.test(mascara.value)) {
    ...
    let host2 = Math.pow(2,24)/subredes.value;
    let clase = ip.value.split(".")[0];
    let bits = Math.log2(subredes.value);
    if (clase <= 127 && mascaraA.test(mascara.value)) {
      mascaraSubred.value =
        mascara.value.split(".")[0] +
        "." +
        convertirIP(host2 * (subredes.value - 1));
      ...
      if (subredes.value <= 256) {
        mascaraSubred.value = `${mask}.${
          (host2 * (subredes.value - 1)) / 256 / 256
        }.0`;
      } else {
        mascaraSubred.value = `${mask}.255.${
          (((subredes.value - 1) * host2) / 256) % 256
        }`;
      }
    } else if (clase >= 192 && clase <= 223 && mascaraC.test(mascara.value)) {
      //mascara clase C
